...
\hlstd{fourStats} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{data}\hlstd{,} \hlkwc{lev} \hlstd{=} \hlkwa{NULL}\hlstd{,} \hlkwc{model} \hlstd{=} \hlkwa{NULL}\hlstd{) \{}
    \hlcom{# This code will use the area under the ROC curve and the sensitivity and}
    \hlcom{# specificity values from the built in twoClassSummary function}
    ...
    \hlcom{# The best possible model has sensitivity of 1 and specifity of 1. How far}
    \hlcom{# are we from that value?}
    \hlstd{coords} \hlkwb{=} \hlkwd{matrix}\hlstd{(}\hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,} \hlnum{1}\hlstd{, out[}\hlstr{"Spec"}\hlstd{], out[}\hlstr{"Sens"}\hlstd{]),} \hlkwc{ncol} \hlstd{=} \hlnum{2}\hlstd{,} \hlkwc{byrow} \hlstd{=} \hlnum{TRUE}\hlstd{)}
    \hlcom{# return the disctance measure together with the output from two class}
    \hlcom{# summary}
...
{\centering \includegraphics[width=\maxwidth]{knitr_figure/practical3-figure2-1} 
